...
int main()
using namespace std;vectorestring> vostr;string temp;
l/ acquire strings
cout <<"Enter strings (empty line to quit):while igetline lcin, temp) && temp [0] != 'lO')
vostr.push_back ( temp);
cout<< "Here is your input .ln" ;
for_each (vostr.begin ( 1, vostr.end() . ShowStr

// store in a file
ofstream fout( "strings.dat", ios_base: :out | ios
base: :binary);
for_each(vostr.begin() , vostr.end(),store(fout,
fout .close ();
// recover file contents
vectorestring> vistr;
ifstream fin("strings.dat", ios_base::in | ios_base::binary);if ( !fin.is_open ())
{
cerr c"Could not open file for input .in" ;exit(EXIT_FAILURE);
}
Getstrs(fin, vistr);
cout<< "\nHere are the strings read from the file:ln" ;for_each(vistr.begin() , vistr.end( , ShowStr) ;
return 0;

该程序以二进制格式打开文件，并想使用read()和 write()来完成IO。余下的工作如下所述。
